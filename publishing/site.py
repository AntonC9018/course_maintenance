...
def generate_katex_css() -> str:
    """Renderer-only KaTeX stylesheet (rehype-katex output needs it).

    Without this, ``.katex-mathml`` (the accessible MathML copy) is not
    visually hidden, so every formula appears twice (MathML + HTML).
    Bundled via Starlight ``customCss`` so fonts resolve from the pinned
    ``katex`` package offline (no CDN).
    """
    return (
        "/* Generated by course_maintenance `site build` (KaTeX CSS). */\n"
        "/* rehype-katex emits MathML + HTML; this hides the MathML copy. */\n"
...
def generate_astro_config(config, identity, starlight_sidebar,
    ...
    lines = [
        ...
        "      // KaTeX CSS via customCss hides the MathML copy (else formulas",
        "      // appear duplicated: MathML + HTML). Fonts bundle from the",
        "      // pinned katex package, no CDN.",
...
def write_site_project(out_dir: Path, files: dict[str, str], copy_list,
    ...
    # KaTeX stylesheet for math (rehype-katex needs its CSS, else MathML
    # copy stays visible and formulas look duplicated).
    styles_dir = out_dir / "src" / "styles"
    styles_dir.mkdir(parents=True, exist_ok=True)
    (styles_dir / "custom.css").write_text(
        generate_katex_css(), encoding="utf-8")
