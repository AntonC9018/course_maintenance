"""Read-only `publishing check` handler (issue #7 scope: CFG-1..CFG-6).

Validates configuration, selected content, repo identity, and route-mapping
uniqueness without writing source files. Returns 0 on success, 1 on any
validation error. Usage errors (bad --course-repo) are handled by the
publish.py dispatcher (exit 2) before this runs.

Extensibility: future issues (#8-15) add metadata/link/asset/projection
stages as new functions called in order here; keep this handler read-only
and keep error format (source path + rule + corrective operation).
